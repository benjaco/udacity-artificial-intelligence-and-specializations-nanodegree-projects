...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        action_layer = set()
        previous_s_level = self.s_levels[-1]

        for action in self.all_actions:
            action_node = PgNode_a(action)
            # continue if action arent possible
            if not action_node.prenodes.issubset(previous_s_level):
                continue

            for statenode in previous_s_level:
                if statenode in action_node.prenodes:
                    action_node.parents.add(statenode)
                    statenode.children.add(action_node)

            action_layer.add(action_node)

        self.a_levels.append(action_layer)
    ...
    def add_literal_level(self, level):
        ...
        actions = self.a_levels[-1]
        for action in actions:
            for added_state in action.effnodes:
                action.children.add(added_state)
                added_state.parents.add(action)
                state_layer.add(added_state)

        self.s_levels.append(state_layer)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # if node a pos effect in node b neg effect, return true
        # if node a neg effect in node b pos effect, return true

        if contains_shared_item(node_a1.action.effect_add,
                                node_a2.action.effect_rem):
            return True

        if contains_shared_item(node_a1.action.effect_rem,
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # node a's effect is not allowed in node b's precondition
        # node b's effect is not allowed in node a's precondition

        # if node a pos effect is in node b neg precon: return true
        # if node a neg effect is in node b pos precon: return true
        # if node b pos effect is in node a neg precon: return true
        # if node b neg effect is in node a pos precon: return true

        return contains_shared_item(node_a1.action.effect_add,
                                    node_a2.action.precond_neg) or \
               contains_shared_item(node_a1.action.effect_rem,
                                    node_a2.action.precond_pos) or \
               contains_shared_item(node_a2.action.effect_add,
                                    node_a1.action.precond_neg) or \
               contains_shared_item(node_a2.action.effect_rem,
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            goal_found = False
            for level_depth, node in [(level_depth, node) for (level_depth, state_level) in enumerate(self.s_levels) for node in state_level]:
            ...
            if not goal_found:
                # would like to return inf, but it is a float
                return int(10e6)
